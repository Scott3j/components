<script lang="ts">
	...
	let progressY = tweened(0, {
		easing: sineIn,
		duration: 0
	});
	let progressX = tweened(100, {
		easing: sineIn,
		duration: 0
	});
...
<main>
	<div class="panel">
		...
		<label>
			...
			<input
				type="checkbox"
				bind:checked={options.applyUserSelectHack}
			/>
		</label>
		...
		<div>
			Default class: <input
				bind:value={options.defaultClass}
			/>
		</div>
		...
		<div>
			<label>
				...
				<input
					type="checkbox"
					bind:checked={options.ignoreMultitouch}
				/>
			</label>
		</div>
